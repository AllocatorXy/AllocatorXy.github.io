...
3. 发送请求到服务器
4. 监控ajax状态，根据`readystate`判断ajax是否完成，根据http状态码确定请求是否成功:
 ...
 * json.data --  json           post用的数据
 * json.cbName  -- string       接口中用于获得回调函数名的参数
...
/**
 * usage
 *  以百度搜索的jsonp接口为例: http://suggestion.baidu.com/su?wd=
 *  oT.value: 文本框的值
 *  jsonp({
 *      url: 'http://suggestion.baidu.com/su?wd=' + oT.value,
 *      cbName: 'cb',
 *      success: //....
 *  });
 */
...
```js
/**
 * 还是以百度搜索接口为例: http://suggestion.baidu.com/su?wd=
 * ajax.url  --  string             接口地址
 * ajax.jsonp  -- string            接口中用于获得回调函数名的参数
 * ajax.jsonpCallback  --  string   自定义回调函数名, 默认jq会产生一个随机的函数名
 */
$('input').keyup(() => {
    $.ajax({
